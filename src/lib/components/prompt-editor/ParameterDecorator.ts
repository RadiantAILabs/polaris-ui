...
class DeleteButtonWidget extends WidgetType {
	...
	constructor(value: string, from: number, to: number, view: EditorView, error: boolean = false) {
		super();
		this.value = value;
		this.from = from;
		this.to = to;
	...
	override eq(other: DeleteButtonWidget): boolean {
		return (
...
export function createParameterDecorator(getInvalidParams: () => Set<string> = () => new Set()) {
	return ViewPlugin.fromClass(
		class {
			...
			findDecorators(view: EditorView) {
